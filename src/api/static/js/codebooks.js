...
if (typeof window.currentCodebookDetail === "undefined")
...
async function loadCodebooks() {
  var year =
    document.getElementById("codebook-year-filter") &&
    document.getElementById("codebook-year-filter").value;
  var source =
    document.getElementById("codebook-source-filter") &&
    document.getElementById("codebook-source-filter").value;
  ...
  try {
    ...
    list.innerHTML = codebooks
      .map(function (cb) {
        ...
        return (
          '<div class="card codebook-card" data-year="' +
          yr +
          '" data-source="' +
          src +
          ...
          escapeHtml(cb.source) +
          '</h3><p class="card-subtitle">Year: ' +
          cb.year +
          "</p></div>" +
          (cb.release_type
            ? '<span class="badge badge-primary">' +
              escapeHtml(cb.release_type) +
              "</span>"
            : "") +
          "</div>" +
          '<div class="card-meta">' +
          '<div class="meta-item"><span class="meta-label">Variables:</span><span>' +
          (cb.total_variables ?? 0) +
          "</span></div>" +
          '<div class="meta-item"><span class="meta-label">Sections:</span><span>' +
          (cb.total_sections ?? 0) +
          "</span></div>" +
          '<div class="meta-item"><span class="meta-label">Levels:</span><span>' +
          ((cb.levels || []).join(", ") || "N/A") +
          "</span></div>" +
          "</div>" +
...
function renderCoreVariablesBatch(
  container,
  variables,
  year,
  source,
...
function showMoreCoreVariables() {
  ...
  renderCoreVariablesBatch(
    container,
    _coreVariablesCache,
    _coreVariablesYear,
    _coreVariablesSource,
    _coreVariablesShown,
    CORE_VARIABLES_BATCH,
  );
  ...
  if (loadMoreWrap) {
    if (_coreVariablesShown >= _coreVariablesCache.length) {
      ...
      if (btn)
        btn.textContent =
          "Load more (" +
          (_coreVariablesCache.length - _coreVariablesShown) +
          " remaining)";
    }
  }
}
...
function setCoreDetailTab(tab) {
  ...
  if (sectionsEl)
    sectionsEl.style.display = tab === "sections" ? "block" : "none";
  if (variablesEl)
    variablesEl.style.display = tab === "variables" ? "block" : "none";
}
...
async function viewCodebook(year, source) {
  ...
  titleEl.textContent =
    window.currentCodebookDetail.source +
    " – " +
    window.currentCodebookDetail.year;
  ...
  try {
    var sections = await apiCall(
      "/sections?year=" +
        window.currentCodebookDetail.year +
        "&source=" +
        encodeURIComponent(window.currentCodebookDetail.source),
    );
    ...
    if (variables.length === 0) {
      variablesEl.innerHTML =
        '<div class="detail-list-wrap"><div class="empty-state">No variables</div></div>';
    } else {
    ...
    sectionsEl.innerHTML =
      '<div class="detail-list-wrap"><div class="error">' +
...
function viewCoreVariableInModal(el) {
  ...
  if (!source && window.currentCodebookDetail)
    source = window.currentCodebookDetail.source;
  source = source || "hrs_core_codebook";
