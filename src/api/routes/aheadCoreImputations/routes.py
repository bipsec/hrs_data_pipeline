...
router = APIRouter(prefix="/api/v1/hrs/ahead-core-imputations", tags=["Codebooks"])

@router.get("/codebooks", response_model=List[CodebookSummary])
async def get_codebooks(
    year: Optional[int] = Query(None, description="Filter by year"),
    source: Optional[str] = Query(None, description="Filter by source (e.g., ahead_core_imputations_codebook)"),
...
async def get_codebook_by_year(
    ...
    source: str = Query("ahead_core_imputations_codebook", description="Source name"),
):
