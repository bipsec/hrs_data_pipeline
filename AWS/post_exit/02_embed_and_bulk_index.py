"""Embed HRS Post-Exit data description chunks and bulk-index into OpenSearch on JetStream2."""
import json
import os
import sys
...
default_data_dir = script_dir.parent / "data" / "post_exit"
POST_EXIT_DATA_DIR = os.getenv("POST_EXIT_DATA_DIR", str(default_data_dir))
...
def main():
    if not Path(POST_EXIT_DATA_DIR).is_dir():
        print(f"ERROR: Post-exit data directory not found: {POST_EXIT_DATA_DIR}")
    ...
    for chunk in tqdm(iter_chunks(POST_EXIT_DATA_DIR), desc="Embed + index (post_exit)"):
        chunk_id = chunk.get("chunk_id")
        content = chunk.get("content", "").strip()

        if not chunk_id or not content or len(content) < 10:
            skipped += 1
            continue
    ...
    print(f"\nDone — {total} post-exit chunks indexed, {skipped} skipped.")
