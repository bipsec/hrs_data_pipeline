"""Embed HRS Exit data description chunks and bulk-index into OpenSearch on JetStream2."""
import json
import os
import sys
...
default_data_dir = script_dir.parent / "data" / "exit"
EXIT_DATA_DIR = os.getenv("EXIT_DATA_DIR", str(default_data_dir))

OPENSEARCH_HOST = os.getenv("OPENSEARCH_HOST", "http://localhost:9200")
OPENSEARCH_USER = os.getenv("OPENSEARCH_USER", "admin")
OPENSEARCH_PASSWORD = os.getenv("OPENSEARCH_PASSWORD", "admin")
OS_INDEX = os.getenv("OS_INDEX", "hrs_exit_data_desc")
...
def main():
    if not Path(EXIT_DATA_DIR).is_dir():
        print(f"ERROR: Exit data directory not found: {EXIT_DATA_DIR}")
    ...
    for chunk in tqdm(iter_chunks(EXIT_DATA_DIR), desc="Embed + index (exit)"):
    ...
    print(f"\nDone — {total} exit chunks indexed, {skipped} skipped.")
