"""Hybrid (BM25 + KNN) search over HRS Exit data description chunks in OpenSearch."""
import argparse
import os
...
def _hit_to_result(rank, h, score):
    ...
    return {
        "rank": rank, "data_source": "exit", "index": OS_INDEX,
...
def hybrid_search_logged(query: str, filters: Optional[Dict[str, Any]] = None):
    logger.info("===== HRS EXIT HYBRID SEARCH (data_desc) START =====")
    ...
    if not fused_ids:
        logger.info("===== HRS EXIT HYBRID SEARCH END (no results) =====\n")
        return []
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Hybrid search over HRS Exit data descriptions (OpenSearch)")
